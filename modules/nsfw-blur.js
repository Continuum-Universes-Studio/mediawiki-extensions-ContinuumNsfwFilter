(function () {
  ...
  const isFilePage = isTruthy(mw.config.get('wgNSFWFilePage'));
  const currentPageTitle = normalizeFileTitle(
    String(mw.config.get('wgPageName') || '').replace(/_/g, ' ')
  );
  ...
  function rewriteFilePageImage(img) {
    if (
      !isFilePage ||
      !placeholderImageUrl ||
      !currentPageTitle ||
      !img ||
      img.dataset.nsfwPlaceholderApplied === '1'
    ) {
      return;
    }
    ...
    if (img.hasAttribute('data-src')) {
      img.setAttribute('data-src', placeholderImageUrl);
    }
  ...
  function markImageIfNSFW(img) {
    if (!img || img.classList.contains('nsfw-blur') || img.dataset.nsfwPlaceholderApplied === '1') return;
